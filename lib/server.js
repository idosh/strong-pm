...
function Server(originalCommand, baseDir, listenPort, controlPath) {
  ...
  // XXX(sam) main app would make more sense to me, but even the use of the word
  // 'app' for express objects/middleware is dubious: because we also manage
  // 'apps', which are different apps...
  ...
  // XXX(sam) mesh app would make more sense to me, or mesh Middleware?
  this._app = meshServer(this._serviceManager);

  // XXX(sam) soon there will be other options, like --trace
  this._startOptions = {
  ...
  // XXX(sam) used to be a property on the express app, legacy? I found no refs
  ...
  var ctlOptions = {
    server: this, // XXX(sam) we pass server, why also pass individual props?
...
Server.prototype.getStartCommand = function() {
  return util.format('sl-run --cluster=%s', this._startOptions.size);
};
