...
function ServiceManager(server, enableTracing) {
  this.server = server;
  this.enableTracing = enableTracing;
  if (enableTracing)
    configDefaults.start += ' --trace';
}
...
ServiceManager.prototype.onInstanceUpdate =
  function onInstanceUpdate(instance, callback) {
    var traceFlag = '';
    if (this.enableTracing) traceFlag = '--trace';
    ...
    configDefaults.start =
      [util.format('sl-run --cluster=%s %s', instance.cpus, traceFlag)];
